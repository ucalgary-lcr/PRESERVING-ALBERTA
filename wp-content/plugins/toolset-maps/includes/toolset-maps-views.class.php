...
class Toolset_Addon_Maps_Views {
	...
	public function delete_stored_assets_on_delete_attachment( $attachment_id ) {
        ...
        //TODO: after implementing toolsetmaps-135, search for marker_images with relative url too.
	}
		...
		switch ( $action ) {
			...
			case 'delete':
				if ( in_array( $target_url_relative, $saved_options['map_style_files'] ) ) {
                    ...
                    //$this->delete_saved_option( $this->make_full_upload_link_relative( $target_url), 'map_style_files', $saved_options);
				    if ( $deleted === false ) {
				        wp_send_json_error();
                    }
				} else {
	...
	function wpv_map_render_shortcode( $atts, $content='' ) {
